...
from grief.logging import RotatingFileHandler
...
class CogsUtils:
    ...
    # def __init__(
    #     self, cog: typing.Optional[commands.Cog] = None, bot: typing.Optional[Grief] = None
    # ) -> None:
    #     if cog is not None:
    #         if isinstance(cog, str):
    #             cog = bot.get_cog(cog)
    #         self.cog: commands.Cog = cog
    #         self.bot: Grief = self.cog.bot if hasattr(self.cog, "bot") else bot
    ...
    #     else:
    #         self.cog: typing.Optional[commands.Cog] = None
    #         self.bot: typing.Optional[Grief] = None
    ...
    # async def add_cog(
    #     self, bot: typing.Optional[Grief] = None, cog: typing.Optional[commands.Cog] = None, **kwargs
    # ) -> commands.Cog:
    #     """
    ...
    def get_logger(
        ...
        if name is not None and cog is None:
            return (
                ...
                if name.startswith("Grief.")
                else logging.getLogger(f"Grief.AAA3A-cogs.{name}")
            )

        logger = (
            logging.getLogger(f"Grief.{cog.__repo_name__}.{cog.qualified_name}")
            if name is None
            else logging.getLogger(f"Grief.{cog.__repo_name__}.{cog.qualified_name}.{name}")
        )
    ...
    async def get_cog_version(
        cls, bot: Grief, cog: typing.Union[commands.Cog, str]
    ...
    @classmethod
    async def add_hybrid_commands(cls, bot: Grief, cog: commands.Cog) -> None:
    ...
    @classmethod
    async def get_hook(cls, bot: Grief, channel: discord.TextChannel) -> discord.Webhook:
        """
        ...
        if hook is None:
            hook = await channel.create_webhook(name=f"grief_webhook_{str(channel.id)}")
        return hook
    ...
    @classmethod
    def get_all_repo_cogs_objects(cls, bot: Grief) -> typing.Dict[str, commands.Cog]:
        """
    ...
    @classmethod
    def at_least_one_cog_loaded(cls, bot: Grief) -> bool:
        """
