...
class ClownboardEntry:
    ...
    def check_roles(self, member: Union[discord.Member, discord.User]) -> bool:
        ...
        ----------
            member: Union[discord.Member, discord.User]
                The member object which added the reaction for this clownboard.

        Returns
        -------
            bool
                Whether or not this member is allowed to utilize this clownboard.
        """
        if not isinstance(member, discord.Member):
            # this will account for non-members reactions and still count
            # for the clownboard count
            return True
        guild = member.guild
        whitelisted_roles = [
        ...
        if whitelisted_roles:
            ...
            # roles can clownboard something
    ...
    def check_channel(self, bot: Red, channel: discord.TextChannel) -> bool:
        """
        Checks if the channel is allowed to track clownboard
        messages
    ...
    async def from_json(cls, data: dict, guild_id: Optional[int]):
        ...
        if guild is None and guild_id is not None:
            guild = guild_id
        clownboarded_messages = data.get("clownboarded_messages", {})
        if isinstance(messages, list):
            new_messages = {}
            async for message_data in AsyncIter(messages, steps=500):
                message_obj = ClownboardMessage.from_json(message_data, guild)
            ...
            async for key, value in AsyncIter(messages.items()):
                msg = ClownboardMessage.from_json(value, guild)
                new_messages[key] = msg
            messages = new_messages
        if not clownboarded_messages:
...
@dataclass
class ClownboardMessage:
    """A class to hold message objects pertaining
    To clownboarded messages including the original
    message ID, and the clownboard message ID
    ...
    def __repr__(self) -> str:
        return (
            "<clownboardMessage author={0.author} guild={0.guild} count={1} "
    ...
    async def update_count(
        self, bot: Red, clownboard: ClownboardEntry, remove: Optional[int]
    ) -> None:
        """
        This function can pull the most accurate reaction info from a clownboarded message
        ...
        if orig_channel:
            try:
                ...
                orig_reaction = [
                    r for r in orig_msg.reactions if str(r.emoji) == str(clownboard.emoji)
                ]
        ...
        if new_channel:
            try:
                ...
                new_reaction = [
                    r for r in new_msg.reactions if str(r.emoji) == str(clownboard.emoji)
                ]
            except discord.HTTPException:
                pass
